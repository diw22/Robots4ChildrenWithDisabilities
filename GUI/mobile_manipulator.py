...
import torch
import zmq
from lerobot.common.robot_devices.robots.xbox_input import XboxController
import pygame
import time
#import mediapipe
...
class MobileManipulator:
    ...
    def on_release(self, key):
        try:
            ...
            pass
    def xbox_keys(self):
        #hat_x, hat_y = self.xbx.controller.get_hat(0)
        hat_x, hat_y = self.xbx.controller.get_hat(0)

        if self.xbx.controller.get_button(1):
            if hat_y == 1:
                self.speed_index = min(self.speed_index + 1, 2)
                print(f"Speed index increased to {self.speed_index}")
                time.sleep(1)
...
class LeKiwi:
    ...
    def set_velocity(self, command_speeds):
        ...
        """
        print(f"[DEBUG] Setting wheel velocities: {command_speeds}")
