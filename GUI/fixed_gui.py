import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QStackedWidget
from PyQt5.QtCore import Qt

from input_manager import input_manager
from control_modes_basic import controller_manager
from GUI_draft_input_old import MainMenu, TicTacToe, MessageWidget, FreeRoamWidget

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("My Robot GUI")
        # Set a reasonable default size and position
        self.setGeometry(100, 100, 800, 600)

        # Create the central stacked widget
        self.stack = QStackedWidget(self)
        self.setCentralWidget(self.stack)

        # Instantiate pages
        self.main_menu = MainMenu(self.stack)
        self.tic_tac_toe = TicTacToe(self.stack)
        self.message_widget = MessageWidget(self.stack)
        self.free_roam_widget = FreeRoamWidget(self.stack)

        # Add pages to the stack
        self.stack.addWidget(self.main_menu)
        self.stack.addWidget(self.tic_tac_toe)
        self.stack.addWidget(self.message_widget)
        self.stack.addWidget(self.free_roam_widget)

        # Initial input mode
        input_manager.set_input_type("controller")
        # Activate controller for the starting page
        self.main_menu.activate_controller()

        # Listen for page selections if MainMenu emits a signal
        try:
            self.main_menu.page_selected.connect(self.on_page_selected)
        except AttributeError:
            pass

        # Store PS5 control running state
        self._ps5_active = False

    def on_page_selected(self, page_index):
        """Handler for switching pages and input modes."""
        # Stop any existing input
        input_manager.stop()
        # Stop PS5 control if it was active
        if self._ps5_active:
            try:
                from draft_ps5_control_thread import stop_ps5_control
                stop_ps5_control()
            except ImportError:
                pass
            self._ps5_active = False

        # Switch the visible page
        self.stack.setCurrentIndex(page_index)
        widget = self.stack.currentWidget()

        # If entering Free Roam, start PS5 control thread
        if widget is self.free_roam_widget:
            from draft_ps5_control_thread import start_ps5_control
            start_ps5_control()
            self._ps5_active = True
        else:
            # Ensure controller mode and activate widget's controller callbacks
            input_manager.set_input_type("controller")
            try:
                widget.activate_controller()
            except AttributeError:
                pass

if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = MainWindow()
    window.show()  # Show a normal window with title bar and close button
    sys.exit(app.exec_())
