...
To get started with this project, clone the GitHub repository and set up the catkin workspace as follows:

```bash
# 1. Clone the repository
cd ~
git clone https://github.com/diw22/Robots4ChildrenWithDisabilities.git

# 2. Navigate into the workspace
cd ~/Robots4ChildrenWithDisabilities/prototypes/ros/catkin_ws
```

## 🧱️ Initial Setup (for collaborators)

After cloning the repository, follow these steps to initialize the workspace:
...
# 2. Go back and build the workspace
cd ..
catkin_make
...
To avoid repeating the last step every time, add it to your `.bashrc`:

```bash
echo "source ~/Robots4ChildrenWithDisabilities/prototypes/ros/catkin_ws/devel/setup.bash" >> ~/.bashrc
source ~/.bashrc
```
